...
## Git LFS運用ルール（開発者向け）
このリポジトリは `.jar` `.exe` `.ttf` などの大容量バイナリを **Git LFS** で管理しています。  
PR作成前に、以下を必ず確認してください。

1. LFS有効化: `git lfs install`
2. 取得確認: `git lfs pull`
3. 追跡確認: `git lfs ls-files`
4. 追加したLFS対象ファイルの push 確認: `git lfs push --all origin <作業ブランチ名>`

> [!IMPORTANT]
> LFSオブジェクトの push 漏れがあると、他環境で `FFDec` や `JRE` が破損状態になり、SWF関連処理や統合テストが失敗します。


## FFDec/JRE 健全性チェック（開発者向け）
SWF関連の作業前に、最低限以下のコマンドで起動確認してください。

```powershell:
$ java -jar data\ffdec\ffdec.jar -help
```

期待結果: FFDec のヘルプが表示されること（`NoClassDefFoundError` などが出ないこと）。


## ドキュメント方針
`README.md` は一般ユーザー向けドキュメントとし、配布導線は **Releaseダウンロード（LFSを介さない利用）** を前提とします。  
開発者向けの詳細手順（LFS運用、テスト前チェック、ビルド手順など）は本 `CONTRIBUTING.md` に記載してください。
...
## フォント加工レシピの作成手順
### 補間フォントのオフセット確認
* スカイリムのデフォルトEverywhereフォントのプレビュー画像を控えておく。

* 対象のフォントを入力し、以下のパラメータを設定する。
   * 上端: 900
   * 下端: -124
   * 行間: 0
   * 下線の位置: 0
   * 下線の太さ: 50
   * サブセットフォント: `data/subsets/subset_jp_skyrim_custom.txt`

* フォントのプレビュー（`0Aa永あ国`）で文字の太さを、デフォルトプレビュー画像と見比べながら太さ調整を行う。
  * 注: 太いフォントは細らせるとデザインが破綻するため、あくまでデフォルト比で細い（＝見辛い）ものに対してのみ行う。

* 太さ調整後、上端/下端位置とグリフの位置を確認し、ズレがひどければ縦オフセット値で中心になるように調整する。

* マージフォントを設定する。

* 出力を行い、フォント情報にてサブセット欠落が無いことを確認する。あればマージフォントで埋める。
   * 注: 令和(特殊文字)などなくても特段問題がないものは欠落を許容する。
