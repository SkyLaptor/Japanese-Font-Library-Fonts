...
def process_swf(params: Mapping[str, Any]) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for index, item in enumerate(items, start=1):
            ...
            if is_otf_path(font_path):
                ...
                with TTFont(str(font_path)) as source_font_obj:
                    ...
                    temp_ttf_path = (
                        temp_root / f"embed_{index:02d}_{font_path.stem}.ttf"
                    )
        ...
        if len(prepared_items) == 1:
            ...
            # 位置(1..N)基準の内部名マップを生成（XML拡張段階で反映する）
            internal_names_by_pos = {
            ...
            replace_glyphs_in_swf(
                ...
                output_swf_path,
                ttf_paths,
                font_names_by_position=internal_names_by_pos,
            )
            # 多フォント時は巨大SWFへのswf2xml往復を避けるため、
            # 名前反映はXML拡張段階で済ませる（事後パッチは行わない）
